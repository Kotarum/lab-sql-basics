...
select DISTINCT  status  FROM loan l 
order by status ASC ;
...
SELECT account_id, amount FROM loan l 
order by account_id 
LIMIT 5;
...
select trans_id , `date` , `type` , amount  FROM trans t 
...
SELECT `date` , COUNT(loan_id) as loan_count  FROM loan l 
WHERE `date` < 930907
GROUP BY `date` 
ORDER BY `date` DESC ;
...
SELECT `date`, duration, COUNT(loan_id) as loan_count  FROM loan l 
...
SELECT account_id, `type` , SUM(amount) as total_amount FROM trans t 
